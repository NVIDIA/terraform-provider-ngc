		...
		Steps: []resource.TestStep{
			// Verify Function Creation
			{
				Config: fmt.Sprintf(`
					...
					testutils.TestTracesTelemetryId,
				),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet(testCloudFunctionResourceFullPath, "version_id"),
					...
					resource.TestCheckResourceAttr(testCloudFunctionResourceFullPath, "telemetries.traces_telemetry_id", testutils.TestTracesTelemetryId),
				),
			},
			// Verify Function Update again won't change anything
			{
